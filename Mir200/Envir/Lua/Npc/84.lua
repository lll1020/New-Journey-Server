...
local function _get_state(play)
    ...
    for i = 1, #((_config and _config.rune_order) or {}) do
        local key = tostring((_config.rune_order or {})[i])
        if data.runes[key] == nil and data.runes[i] ~= nil then
            data.runes[key] = data.runes[i]
        end
        data.runes[i] = nil
    end
...
local function _has_all_lingshou(play)
    ...
    local ls = data.ls or {}
    for i = 1, #ls_cfg do
        local value = ls[tostring(i)]
        if value == nil then
            value = ls[i]
        end
        if _toint(value) <= 0 then
            return false
        end
    end
    return true
...
local function _has_double_shendao(play)
    local compat = (_config and _config.compat) or {}
    local god_data = Player.getJsonTableByVar(play, VarCfg["T_神道之路"] or "T_神道之路") or {}
    local gods = type(god_data.gods) == "table" and god_data.gods or {}
    local god_one = gods["1"] or gods[1] or {}
    local god_two = gods["2"] or gods[2] or {}
    if _toint(god_one.cert) >= 1 and _toint(god_two.cert) >= 1 then
        return true
    end

    -- Keep the original task-progress check for players with legacy saves.
...
local function _has_join_yehuo(play)
    ...
    for _, key in ipairs(compat.yehuo_flags or {}) do
        ...
        -- The active replacement NPCs do not write T_dljq. Their crafted
        -- item is reliable completion evidence whether it is in bag or gear.
        local cfg = (teshudata or {})[key] or {}
        local item_name = tostring(cfg.item_name or "")
        if item_name ~= "" then
            if _toint(getbagitemcount(play, item_name)) > 0 then
                return true
            end
            for _, pos in ipairs(_equip_slots) do
                if Player.hasEquipOnPos(play, pos, item_name) then
                    return true
                end
            end
        end
...
local function _refresh_count(data)
    ...
    for _, idx in ipairs((_config and _config.rune_order) or {}) do
        local runeValue = (data.runes or {})[tostring(idx)]
        if runeValue == nil then
            runeValue = (data.runes or {})[idx]
        end
        if _toint(runeValue) == 1 then
            count = count + 1
        end
    end
    data.all = count
