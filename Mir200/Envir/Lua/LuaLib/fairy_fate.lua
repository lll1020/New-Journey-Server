...
local function _normalize_recharge_done(play, state)
    local changed = false
    local rechargeTotal = _toint(getplaydef(play, VarCfg["U_真实充值"]))
    for _, detail in ipairs(_cfg.details or {}) do
...
local function _build_snapshot(play, state)
    ...
    return {
        ...
        recharge_total = _toint(getplaydef(play, VarCfg["U_真实充值"])), xianfu_level = _toint(xianfu.level), xianfu_xianghua = _toint(xianfu.stats.xiangHua),
...
local function _reached(play, state, snap, detail)
    ...
    if kind == "xianfu_level" then return snap.xianfu_level >= r.target end
...
local function _evaluate(play, state) -- 扫描全部成就条件，达成后即时发奖励与提示
    ...
    for _, detail in ipairs(_cfg.details or {}) do
        ...
        if not _is_retired_legacy_rule(detail)
            ...
            -- Player.sendmsgEx(play, "恭喜达成|【" .. tostring(detail.name or detail.id) .. "】#218|成就")
        ...
        for _, milestone in ipairs(_cfg.milestones or {}) do
            if _toint(milestone.count) == target then
                ...
                if milestone.reward_cfg and milestone.reward_cfg.title and milestone.reward_cfg.title ~= "" then
                    if not _can_grant_milestone_title(state, target) then Player.sendmsgEx(play, "请按顺序领取前置成就卷轴称号#57") return end
                end
