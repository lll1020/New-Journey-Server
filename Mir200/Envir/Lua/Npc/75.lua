...
function npc.link(play, npcid, p2, p3, msgData)
    ...
    if p2 == 1 then
        ...
        if equipname ~= _config.details[json_data.idx].now then
            Player.sendmsgEx(play, "请先装备#57|【".._config.details[json_data.idx].now.."】#218|进行升级#57")
            return
        end
        if equipname == _config.details[json_data.idx].give then
            Player.sendmsgEx(play, "你的#57|【".._config.details[json_data.idx].name.."】#218|已经是最高级别，无法继续升级#57")
            return
        end
        ...
        Player.sendmsgEx(play,  "恭喜你，|【".._config.details[json_data.idx].name.."升级成功，当前为".._config.details[json_data.idx].give.."】#218|")
