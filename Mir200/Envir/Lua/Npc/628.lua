...
local function _can_enter_main(play, jq_data)
    if tonumber(getplaydef(play, VarCfg.U_zxrw[1]) or 0) ~= 39 then
        Player.sendmsgEx(play, "当前主线还不能进入妄灾讨伐线#57")
        return false
    end
    ...
    if tonumber(jq_data["npc_626"] or 0) < 2 or tonumber(jq_data["npc_621"] or 0) < 2 then
        Player.sendmsgEx(play, "请先完成前置灾厄任务和妄灾线前置任务#57")
        return false
    end
    return true
end
...
function npc.link(play,npcid,ew,aid)
    ...
    if ew == 2 then
        ...
        if state == 0 then
            ...
            Player.sendmsgEx(play, "领取任务：#57|"..item_name.."#249|在#57|"..((_config.prep_task and (_config.prep_task.show_map or _config.prep_task.map)) or "山脉入口").."#249|分别找回左右眼")
...
function npc_628_enter(play)
    ...
    addmirrormap(base_map, dtm, "虚妄山脉", 300,"xtc",136,136)
