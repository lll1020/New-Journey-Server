...
local function _rebuild_item_abil(item_json)
    ...
    for idx, one in ipairs(affixes) do
        table.insert(attr_list, {254, tonumber(one.attr_id or 0) or 0, tonumber(one.value or 0) or 0, 0, 0, idx, idx})
    end
    if #attr_list > 0 then
...
local function _build_client_data(play)
    ...
    local affixes = {}
    if itemobj then
        affixes = _normalize_affixes(_get_item_json(play, itemobj))
    end
    local affix_list = {}
