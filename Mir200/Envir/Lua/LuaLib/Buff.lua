...
Buff = {
    ...
    [313] = function(play,zt) --阴阳玉佩：按时间切换属性（06-18阳：对怪攻速+10%，18-06阴：打怪爆率+10%）
        local function _apply(mode)
            if mode == 1 then
                Player.addattlist(play, "阴阳玉佩_阳", "=", "3#200#1000", 1)
                Player.del_attlist(play, "阴阳玉佩_阴")
            else
                Player.addattlist(play, "阴阳玉佩_阴", "=", "3#242#1000", 1)
    ...
    [101] = function(play,zt) --仙食坊全满
        if zt == 1 then
            Player.addattlist(play, "仙食坊全满", "=", "3#1#8888|3#4#588|3#242#3800|3#244#4888", 1)
        elseif zt == 2 then
...
function Buff.login(play)
    ...
    attrsstr = Player.getAttrTableToStr(attrs)
    Player.addattlist(play, "灵根修炼", "=", attrsstr, 1)
    --兰姐好感度
    if getplaydef(play, VarCfg["U_兰姐好感度"]) > 0 then
        Player.addattlist(play, "兰姐好感度", "=", "3#"..teshudata["npc_13"].attrID.."#"..teshudata["npc_13"].config[getplaydef(play, VarCfg["U_兰姐好感度"])].ratio, 1)
    end
    --福娃猜拳切割
    ...
    if fuwa_cut > 0 then
        Player.addattlist(play, "福娃猜拳切割", "=", "3#" .. (teshudata["npc_66"].cut_attr or 244) .. "#" .. fuwa_cut, 1)
    end
