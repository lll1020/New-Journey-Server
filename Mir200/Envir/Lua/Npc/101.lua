...
local function _find_1002_idx_by_name(listName, itemName)
    local cfg1002 = teshudata and teshudata["npc_1002"]
    local cfgList = (((cfg1002 or {}).details or {})[listName]) or {}
    for idx, cfg in ipairs(cfgList) do
        if cfg and cfg.name == itemName then
            return idx
        end
    end
    return 0
end
local function _resolve_pool_idx(poolCfg, listName)
    local idx = tonumber(poolCfg and poolCfg.idx or 0) or 0
    local cfg1002 = teshudata and teshudata["npc_1002"]
    local cfgList = (((cfg1002 or {}).details or {})[listName]) or {}
    if idx > 0 and cfgList[idx] and (not poolCfg.name or cfgList[idx].name == poolCfg.name) then
        return idx
    end
    if poolCfg and poolCfg.name then
        return _find_1002_idx_by_name(listName, poolCfg.name)
    end
    return idx
end
local function _count_pool_owned(ownedTbl, pool, listName)
    local num = 0
    local used = {}
    for _, cfg in ipairs(pool or {}) do
        local idx = _resolve_pool_idx(cfg, listName)
        if idx > 0 and not used[idx] and tonumber((ownedTbl or {})[tostring(idx)]) == 1 then
            used[idx] = true
            num = num + 1
        end
    end
...
local function _refresh_bonus(play, T_data)
    ...
    local fashionCount = _count_pool_owned(unlockData.yjs, _config.fashion_pool, "sz")
    ...
    local footstepCount = _count_pool_owned(unlockData.yjszj, _config.footstep_pool, "zj")
    if footstepCount >= 5 then
        _merge_attrs(attrs, _config.footstep_bonus[5].attr)
        setskilldeccd(play, _config.footstep_bonus[5].skill.name, "=", 0)
...
local function _grant_fashion(play, T_data, reward, reason)
    ...
    local unlockData = _get_1002_unlock_data(play)
    if tonumber(unlockData.yjs[idx]) == 1 then
        local dupReward = _config.duplicate_fashion_reward or {{"元宝", 660000}}
        Player.rwjl(play, dupReward, reason or ",fashion_duplicate", 1, 999)
        local rewardName = dupReward[1] and dupReward[1][1] or "元宝"
        local rewardNum = dupReward[1] and dupReward[1][2] or 660000
        return tostring(rewardName) .. "*" .. tostring(rewardNum) .. "(重复时装分解)"
    end
...
local function _build_panel_data(play)
    ...
    data.fashion_count = _count_pool_owned(unlockData.yjs, _config.fashion_pool, "sz")
    data.footstep_count = _count_pool_owned(unlockData.yjszj, _config.footstep_pool, "zj")
...
local function _open_box(play, T_data, boxType, choiceIdx)
    ...
    if getbagitemcount(play, boxName) < 1 then
        return false, tostring(boxName) .. "不足#57"
    end
    if reward.kind ~= "item" or type(reward.give) ~= "table" then
        return false, "该奖励暂不支持通过自选箱领取#57"
    end
    takeitem(play, boxName, 1)
    Player.rwjl(play, reward.give, tostring(boxName), 1, 0)
    return true, _reward_label(reward)
end
...
local function _on_skin_update(play)
    local T_data = _get_data(play)
    _save_data(play, T_data)
    _refresh_bonus(play, T_data)
end
