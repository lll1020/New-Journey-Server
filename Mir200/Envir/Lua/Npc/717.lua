...
local _fire_effect_prefix = "npc717_fire_"
...
local function _clear_fire_map_effects(dtm, boss_x, boss_y, radius)
    if not dtm or dtm == "" then
        return
    end
    radius = tonumber(radius or 3) or 3
    boss_x = tonumber(boss_x or 0) or 0
    boss_y = tonumber(boss_y or 0) or 0
    if boss_x <= 0 or boss_y <= 0 then
        return
    end

    for dx = -radius, radius do
        for dy = -radius, radius do
            delmapeffect(_fire_effect_prefix .. dtm .. "_" .. (boss_x + dx) .. "_" .. (boss_y + dy))
        end
    end
end
...
local function _state_clear(dtm)
    if dtm and dtm ~= "" then
        local boss_pos = _task_cfg.boss_pos or {32, 36}
        local fire_radius = tonumber(_task_cfg.fire_ground_radius or 3) or 3
        _clear_fire_map_effects(dtm, tonumber(boss_pos[1] or 32) or 32, tonumber(boss_pos[2] or 36) or 36, fire_radius)
        _run_state[dtm] = nil
    end
end
...
local function _fb_cfg()
    ...
    cfg.fire_effect_id = tonumber(_task_cfg.fire_effect_id or 23) or 23
    cfg.fire_ground_effect_id = tonumber(_task_cfg.fire_ground_effect_id or _task_cfg.boss_effect or 92) or 92
    ...
    cfg.fire_ground_radius = tonumber(_task_cfg.fire_ground_radius or 3) or 3
    cfg.fire_ground_keep = tonumber(_task_cfg.fire_ground_keep or 2) or 2
    return cfg
end
...
local function _fire_ground_map(dtm, boss_x, boss_y, effect_id, radius, keep_sec)
    if not dtm or dtm == "" then
        return
    end
    boss_x = tonumber(boss_x or 0) or 0
    boss_y = tonumber(boss_y or 0) or 0
    radius = tonumber(radius or 3) or 3
    keep_sec = tonumber(keep_sec or 2) or 2
    if boss_x <= 0 or boss_y <= 0 then
        return
    end
    ...
    for dx = -radius, radius do
        for dy = -radius, radius do
            local x = boss_x + dx
            local y = boss_y + dy
            mapeffect(_fire_effect_prefix .. dtm .. "_" .. x .. "_" .. y, dtm, x, y, effect_id, keep_sec, 2, 0)
        end
    end
end
...
function npc_717_dsq(xt,play,dtm,data)
    ...
    if boss then
        ...
        -- BOSS附近铺满地火
        _fire_ground_map(dtm, tonumber(cfg.boss_pos[1] or 32) or 32, tonumber(cfg.boss_pos[2] or 36) or 36, cfg.fire_ground_effect_id, cfg.fire_ground_radius, cfg.fire_ground_keep)
        -- 常驻火焰攻击：BOSS持续释放火焰伤害
