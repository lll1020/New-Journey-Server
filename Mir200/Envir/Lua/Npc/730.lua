...
local function _unlock_map(play)
    if _toint((_get_story(play))[_cfg_key]) >= 2 then
        Player.sendmsgEx(play, "该地图权限已激活#57")
        return
    end
    ...
    if not _check_kill(play, _task_cfg.kill_count or 200) then
        return
    end
    if not _consume_cost(play, _task_cfg.submit or {{"元宝", 1280000}}, "," .. (_config.name or "地图权限")) then
        return
    end
    _finish(play, (_config.name or "地图权限") .. "奖励")
end
...
local function _handle(play, npcid, action, aid) _unlock_map(play) end
