...
-- -- 9.8-1 Exclusive-gem refining removed.
...
--     data.refine = {} -- 9.8-1 Discard legacy refining state.
...
-- -- 9.8-1 原 markTaskStarted 中的聚宝盆碎片保底记录逻辑已停用。
-- -- 9.8-1 function TreasureBasin.markTaskStarted(play)
-- -- 9.8-1     local data = _get_state(play)
-- -- 9.8-1     if data.rebuilt >= 1 or data.task_started >= 1 then
-- -- 9.8-1         return false
-- -- 9.8-1     end
-- -- 9.8-1     data.task_started = 1
-- -- 9.8-1     _save_state(play, data)
-- -- 9.8-1     local dropData = Player.getJsonTableByVar(play, VarCfg["T_物品掉落记录"]) or {}
-- -- 9.8-1     dropData["kill_pity_聚宝盆碎片"] = 0
-- -- 9.8-1     Player.setJsonVarByTable(play, VarCfg["T_物品掉落记录"], dropData)
-- -- 9.8-1     return true
-- -- 9.8-1 end
...
-- -- 9.8-1 Legacy refining payload removed.
...
--         -- 9.8-1 Refining red-point checks removed.
...
-- -- 9.8-1 local itemName = tostring(_config.fragment_item or "聚宝盆碎片")
-- -- 9.8-1 fragment_item = itemName,
-- -- 9.8-1 fragment_need = _toint(_config.fragment_count or 20),
-- -- 9.8-1 fragment_have = getbagitemcount(play, itemName),
-- local function _build_task_payload(play)
--     local data = _grant_artifact_if_needed(play, _get_state(play))
--     -- 9.8-1 fragment_item/fragment_need/fragment_have 任务材料字段已停用。
--     return {
--         mode = "task",
--         T_data = data,
...
-- end
-- -- 9.8-1 local itemName = tostring(_config.fragment_item or "聚宝盆碎片")
-- -- 9.8-1 local needNum = _toint(_config.fragment_count or 20)
-- -- 9.8-1 local haveNum = getbagitemcount(play, itemName)
-- -- 9.8-1 if haveNum < needNum then
-- -- 9.8-1     Player.sendmsgEx(play, "聚宝盆碎片不足")
-- -- 9.8-1     Guard.closeNpcAndAuto(play, npcid)
-- -- 9.8-1     return
-- -- 9.8-1 end
-- -- 9.8-1 Player.takeItemByTable(play, {{itemName, needNum}}, "聚宝盆重铸", nil)
...
-- -- 9.8-1 Exclusive-gem refining actions removed.
...
--     local action = Guard.normalizeAction(play, npcid, p2)
--     if action == nil then return end
--     local allowed = Guard.newActionSet({1, 4, 5, 6, 7, 9})
--     if not Guard.ensureActionAllowed(play, npcid, action, allowed) then return end
--     local data = _get_state(play)
--     if data.rebuilt < 1 and data.activated < 1 then
--         Player.sendmsgEx(play, "请先完成聚宝盆任务")
...
-- -- 9.8-1 聚宝魔石杀怪掉落逻辑及对应 onKillMon 注册已停用；保留其他聚宝值逻辑。
-- -- 9.8-1 local mapName = tostring(getbaseinfo(play, 3) or "")
-- -- 9.8-1 local dl = _continent_by_map_name(mapName)
-- -- 9.8-1 local mobName = tostring(getbaseinfo(mob, 1) or "")
-- -- 9.8-1 local gtype = _toint(guaiwutype and guaiwutype[mobName])
-- -- 9.8-1 if gtype >= 2 or math.random(5000) == 1 then
-- -- 9.8-1     shaguai.temp_drop(play, mob, "聚宝魔石")
-- -- 9.8-1 end
...
-- -- 9.8-1 原材料掉落事件注册：GameEvent.add(EventCfg.onKillMon, TreasureBasin.onKillMon, "聚宝盆")
