...
function buffchufa(play, buffid, zid)
    ...
    elseif buffid == 20103 then
        ...
        if curzuiyi < 100 then
            Player.sendmsgEx(play, string.format("ÄãµÄ×íÒâÖµÎ´´ïÉÏÏÞ|%d#249|£¬ÎÞ·¨Î¬³Ö|×í¾Æ¿ñÄ§Îè#57", _config.max_zuiyi))
            delbuff(play, 20103)
            return
        end
...
function buffchange(play, buffid, zid, lx)
    ...
    elseif buffid == 20103 then
        if getbaseinfo(play,1) == "¾ÆÏÉÃØ¾³" then
            ...
            Player.sendmsgEx(play, "×í¾Æ¿ñÄ§ÎèÒÑÊ§Ð§#57|,ÒÑÀë¿ª|¾ÆÏÉÃØ¾³#249")
        end
