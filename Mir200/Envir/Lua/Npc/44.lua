...
--[[
�ɸ�������
1. �����淨���������� teshudata["npc_44"]������ֻ��ȡ���ò�ִ���߼���
2. ��Ҹ������ݵ�������� VarCfg.T_XianFuData������ά����ʷȫ�������
3. ��ǰ�������°��ɸ������淨������ά���ɰ����а��߼���
]]
...
local DollAttrListName = tostring(DollCfg.attr_list_name or "�ɸ���������")
    ...
    for _, entry in ipairs(plantCfg.product or {}) do
        ...
        if rate >= 100 or (rate > 0 and math.random(100) <= rate) then
            for _, info in ipairs(entry.give or {}) do
                ...
                if lowDanActive and (itemName == "�ɸ���" or itemName == "��ʯ��Ƭ") and itemNum > 0 then
...
local function applyPlantToPlot(play, record, plot, seedId, now)
    ...
    if (tonumber(record.level or 1) or 1) < needLevel then
        return false, string.format("�ɸ��ȼ��ﵽ#57|��%d����#218|����������", needLevel)
    end
...
function Planting.harvest(play, record, params, now)
    ...
    Player.rwjl(play, reward, "�ɸ��ջ�", 1, 0)
...
local function purchase(play, entry, amount, reason)
    ...
    if not ok then
        return false, string.format("%s����", lack or "cost")
    end
...
function Refine.start(play, record, params, now)
    ...
    if not hasRefinePermit(play) then
        return false, string.format("����װ��#218|%s#255|��������", RefineCfg.needEquip or "��������֤")
    end
...
local function levelUp(play, record)
    ...
    local needCost = nextCfg.cost or {}
    if #needCost > 0 then
        Player.takeItemByTable(play, needCost, ",�ɸ�����", nil)
    end
...
--     local pool = PetCfg.identify and PetCfg.identify.bloodlinePool or {"�鶯"}
...
function Doll.draw(play, record, now)
    ...
    if #fixedReward > 0 then
        Player.rwjl(play, fixedReward, "�ɸ����޻�", 1, 0)
    end
...
function ActionHandler.plant(play, npcid, state, params)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, res or "����ʧ��#57")
        return
    end
    Player.sendmsgEx(play, "���ֳɹ�")
    pushAction(play, npcid, "plant", true, "���ֳɹ�", state, res)
end
...
function ActionHandler.plantAll(play, npcid, state, params)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, res or "һ����ֲʧ��#57")
        return
    end
    local seedName = tostring((PlantCfg[tostring((params or {}).seedId or "")] or {}).name or "���")
    local msg = string.format("һ����ֲ��ɣ��Ѳ���%s��%s", tostring(res.count or 0), seedName)
...
function ActionHandler.harvest(play, npcid, state, params)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, res or "��δ����#57")
        return
    end
    Player.sendmsgEx(play, "�ջ����#57")
    pushAction(play, npcid, "harvest", true, "�ջ����", state, res)
end
...
function ActionHandler.buySeed(play, npcid, state, params)
    ...
    Player.sendmsgEx(play, "����ɹ�")
    pushAction(play, npcid, "buySeed", true, "����ɹ�", state, res)
end
...
function ActionHandler.buyEgg(play, npcid, state, params)
    ...
    Player.sendmsgEx(play, "����ɹ�")
    pushAction(play, npcid, "buyEgg", true, "����ɹ�", state, res)
end
...
function ActionHandler.buyMaterial(play, npcid, state, params)
    ...
    Player.sendmsgEx(play, "����ɹ�")
    pushAction(play, npcid, "buyMaterial", true, "����ɹ�", state, res)
end
...
function ActionHandler.refine(play, npcid, state, params)
    ...
    pushAction(play, npcid, "refine", true, "�������", state, res)
end
...
function ActionHandler.levelUp(play, npcid, state, params)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, tostring(res or "�ɸ�����ʧ��") .. "#57")
        pushAction(play, npcid, "levelUp", false, tostring(res or "�ɸ�����ʧ��"), state)
        return
    end
    Player.sendmsgEx(play, string.format("�ɸ������ɹ�����ǰ#57|��%d����#218|", tonumber(state.record.level or 1) or 1))
    pushAction(play, npcid, "levelUp", true, "�ɸ������ɹ�", state, res)
end
...
function ActionHandler.buyDecoration(play, npcid, state, params)
    ...
    Player.sendmsgEx(play, "����ɹ�")
    pushAction(play, npcid, "buyDecoration", true, "����ɹ�", state, res)
end
...
function ActionHandler.equipDecoration(play, npcid, state, params)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, res or "װ���޷���Ч#57")
        return
    end
    Player.sendmsgEx(play, "װ������Ч#57")
    pushAction(play, npcid, "equipDecoration", true, "װ������Ч", state, res)
end
...
function ActionHandler.hatch(play, npcid, state, params)
    ...
    Player.sendmsgEx(play, "�����ɹ�")
    pushAction(play, npcid, "hatch", true, "�����ɹ�", state, res)
end
...
--     persistState(state)
--     if not ok then
--         Player.sendmsgEx(play, res or "ι��ʧ��#57")
--         return
--     end
--     Player.sendmsgEx(play, "ι�����#57")
--     pushAction(play, npcid, "feed", true, "ι�����", state, res)
-- end
...
--     end
--     Player.sendmsgEx(play, "�������#57")
--     pushAction(play, npcid, "identify", true, "�������", state, res)
-- end
...
function ActionHandler.dollDraw(play, npcid, state)
    ...
    persistState(state)
    if not ok then
        Player.sendmsgEx(play, res or "ץ����ʧ��#57")
        return
    end
    Player.sendmsgEx(play, string.format("ץȡ�ɹ���%s", tostring(res.name or "����")))
    pushAction(play, npcid, "dollDraw", true, "ץȡ�ɹ�", state, res)
end

local function handleVisit(play, npcid, state, params, fn)
    local targetName = params and params.targetName
    if not targetName or targetName == "" then
        Player.sendmsgEx(play, "������Ҫ�ݷõ��������#57")
        return
    end
    local actor, record, err = findOnlineTargetByName(targetName)
    if not actor then
        Player.sendmsgEx(play, (err or "�Է�������") .. "#57")
        return
    end
...
function ActionHandler.like(play, npcid, state, params)
    handleVisit(play, npcid, state, params or {}, function(actor, targetRecord)
        ...
        Player.sendmsgEx(play, "���޳ɹ�")
        pushAction(play, npcid, "like", true, "���޳ɹ�", state, res)
    end)
end
...
function ActionHandler.steal(play, npcid, state, params)
    handleVisit(play, npcid, state, params or {}, function(actor, targetRecord, visitParams)
        ...
        persistState(state)
        if not ok then
            Player.sendmsgEx(play, res or "͵ȡʧ��#57")
            return
        end
        Player.sendmsgEx(play, "͵ȡ�ɹ�")
        pushAction(play, npcid, "steal", true, "͵ȡ�ɹ�", state, res)
    end)
end
...
function npc.main(play, npcid)
    ...
    if not (jq_data["npc_55"] and jq_data["npc_55"] >= 2) then
        Player.sendmsgEx(play, "����δ�����ɸ����ݲ��ɽ���#57")
        return
    end
    local state = loadState(play)
    dollRefreshAttr(play, state.record)
...
function npc.link(play, npcid, p2, p3, msgData)
    ...
    if not handler then
        pushAction(play, npcid, payload.action, false, "δ֪����", state)
        return
    end
