...
local function _can_enter_main(play, jq_data)
    if tonumber(getplaydef(play, VarCfg.U_zxrw[1]) or 0) ~= 39 then
        Player.sendmsgEx(play, "当前主线还不能进入息灾讨伐线#57")
        return false
    end
    ...
    if tonumber(jq_data["npc_625"] or 0) < 2 or tonumber(jq_data["npc_622"] or 0) < 2 then
        Player.sendmsgEx(play, "请先完成前置灾厄任务和息灾线前置任务#57")
        return false
    end
    return true
end
...
local function _auto_use_prep_item(play)
    ...
    addbuff(play, 20111)
    Player.sendmsgEx(play, "检测到背包中拥有#57|【"..item_name.."】#218|，已自动获得定身效果#57")
    return true
end
...
function npc_627_delay_back(play)
...
function npc_627_enter(play)
    ...
    addmirrormap(base_map, dtm, "叹息旷野", 300,"xtc",136,136)
...
function npc_627_dsq(xt,play,dtm,data)
    ...
    if getmoncount(dtm,-1,true) < 1 then
        ...
        delaygoto(play, 5000, "@npc_627_delay_back")
    end
end
