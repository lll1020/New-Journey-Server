...
local DROP_RULES = {{map = "冻魂冰窟", item = "上古玄冰", rate = 5000}, {map = "冻魂冰窟", item = "千年陨铁", rate = 3500}, {map = "冻魂冰窟", item = "极寒之心", rate = 8000}, {map = "冻魂冰窟", title = "寒冰剑诀", rate = 10000, require_craft = true}}
...
local function _activate_hanbingjianjue(play)
    local itemobj = _find_item_by_name(play, _task_cfg.weapon or "寒冰剑")
    if not itemobj or itemobj == "0" then
        Player.sendmsgEx(play, "请先持有寒冰剑，再激活寒冰剑诀#57")
        return false
    end
    Player.title_give(play, _task_cfg.skill_title or "寒冰剑诀")
    ...
    Player.sendmsgEx(play, "寒冰剑诀已激活#57")
    return true
end
...
local function _craft(play, reward, stateSuffix)
    local cost = _task_cfg.craft_cost or _task_cfg.submit or _config.cost or {}
    if not _consume_cost(play, cost, "," .. (_config.name or "第六章剧情") .. "合成") then
        return false
    end
    _give_items(play, reward or _task_cfg.craft_reward or _config.jl or _config.rwjl, (_config.name or "第六章剧情") .. "合成")
    if stateSuffix then
        local jq = _get_story(play)
        jq[_cfg_key .. stateSuffix] = 1
        _save_story(play, jq)
    end
    Player.sendmsgEx(play, "合成成功#57")
    return true
end
...
local function _handle(play, npcid, action, aid)
    if action ~= 2 then
        Player.sendmsgEx(play, "收集材料后点击立即合成寒冰剑即可完成任务#57")
        return
    end
    local jq = _get_story(play)
    ...
    if _craft(play, {{"寒冰剑", 1}}, "_craft") then
        jq = _get_story(play)
        jq[_cfg_key .. "_done"] = 1
        _save_story(play, jq)
        _finish(play, (_config.name or "第六章剧情") .. "奖励", true)
        _sg_add(play, NPC_ID)
    end
end
