...
-- reward_desc: 灵兽奥秘：GMD50-100,召唤灵兽时，自身攻击力提高3%

local _cfg_key = "npc_682"
...
function npc.link(play,npcid,ew,aid)
    ...
    if cur_map ~= "灵兽谷" and cur_map ~= "xtc" then
        Player.sendmsgEx(play, "请前往#57|【灵兽谷】#218|完成后再提交#57")
    ...
    if kill_cur < 100 then
        Player.sendmsgEx(play, string.format("击杀不足：#57|【%d/%d】#218|", kill_cur, 100))
        return
    end

    if type(_config.cost) == "table" and #_config.cost > 0 then
