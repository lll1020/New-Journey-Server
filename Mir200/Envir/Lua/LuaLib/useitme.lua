...
function stdmodefunc234(play) ---千里传音 提示：使用50级
    if checkkuafu(play) then
        stop(play)
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>跨服不能使用该物品</font>","Type":9}')
        return
    end
    stop(play)
    if getbaseinfo(play,6) < 60 then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>使用千里传音需要达到60级！</font>","Type":9}')
        return
    end
...
function inputstring23(play) ---
    ...
    if name_len < 1 then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>请输入内容</font>","Type":9}')
        return
    end
    if name_len > 100 then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>内容过长</font>","Type":9}')
        return
    end
    if getbagitemcount(play, "千里传音") < 1 then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>千里传音不足</font>","Type":9}')
        return
    end
    local result, name = exisitssensitiveword(text)
    if result then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>内容包含敏感词</font>","Type":9}')
        return
    end
...
function stdmodefunc30(play, item)
    ...
    if exp >= 10000000 then
        Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#ff0000\'>境界修炼已满级</font>","Type":9}')
        return
    end
    exp = exp + getstditeminfo(getiteminfo(play, item, 2), 8)
    if exp > 10000000 then exp = 10000000 end
    setplaydef(play, VarCfg["U_境界修炼"][2], exp)
    Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#00ff00\'>境界修炼经验+'..getstditeminfo(getiteminfo(play, item, 2), 8)..'</font>","Type":9}')
end
...
function stdmodefunc31(play, item)
    ...
    Player.sendmsgEx(play, 1, '{"Msg":"<font color=\'#00ff00\'>天书杀意值+'..getstditeminfo(getiteminfo(play, item, 2), 8)..'</font>","Type":9}')
end
...
function stdmodefunc32(play, item) --神石召唤
    ...
    if not config or type(config.cost) ~= "table" then
        Player.sendmsgEx(play,1,"神石召唤失败:缺少配置#57")
        return false
    end
    ...
    if not target or not target.list or #target.list == 0 then
        Player.sendmsgEx(play,1,"神石召唤失败:奖池为空#57")
        return false
    end

    local rewardName = target.list[math.random(#target.list)]
    if not rewardName then
        Player.sendmsgEx(play,1,"神石召唤失败:奖品不存在#57")
        return false
    end
