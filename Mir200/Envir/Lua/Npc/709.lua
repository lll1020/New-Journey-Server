...
local function _check_use_pos(play)
    ...
    if cur_map ~= map then
        Player.sendmsgEx(play, "请前往【"..map.."】指定位置使用#57")
        return false
    end
    if x > 0 and y > 0 then
        ...
        if math.abs(px - x) > r or math.abs(py - y) > r then
            Player.sendmsgEx(play, string.format("请在坐标(%d,%d)附近使用，容差%d格#57", x, y, r))
            return false
        end
    end
    return true
...
function npc.link(play,npcid,ew,aid)
    ...
    if state < 1 then
        ...
        if getbaseinfo(play,3) ~= req_map and getbaseinfo(play,3) ~= "xtc" then
            Player.sendmsgEx(play, "请前往【"..req_map.."】接取该任务#57")
            return
        end
        ...
        Player.sendmsgEx(play, "已提交【裂开的酒壶】，获得【".._use_item_name().."】#57")
    ...
    if kill_cur < need then
        Player.sendmsgEx(play, "请先在指定坐标使用【".._use_item_name().."】召唤并击杀BOSS#57")
        return
    end
...
function npc_709_use_item(play, item)
    ...
    Player.sendmsgEx(play, "你使用【".._use_item_name().."】召唤了BOSS【"..boss.."】#57")
    return true
end
