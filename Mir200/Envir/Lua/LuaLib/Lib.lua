...
function login_fhsx(play)
    ...
    if T_zzsj.dqzy then
        if T_zzsj.sh and T_zzsj.sh[""..T_zzsj.dqzy] == 1 then
            setranklevelname(play,"%s\\[踏月々沉默]\\[极·".. constant.zzxg.zy[T_zzsj.dqzy].name.."]\\击杀『"..getplaydef(play,constant.U_srsl).."』")
        else
            setranklevelname(play,"%s\\[踏月々沉默]\\["..constant.zzxg.zy[T_zzsj.dqzy].name.."]\\击杀『"..getplaydef(play,constant.U_srsl).."』")
        end
    else
        setranklevelname(play,"%s\\[踏月々沉默]\\击杀『"..getplaydef(play,constant.U_srsl).."』")
    end
