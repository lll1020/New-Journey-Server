...
shaguai = {
	...
	["340"] = function(play,mob)      --古刹魔瓶：装备背包神器后，击杀怪物有5%概率累计1点打怪切割
        if not Player.hasEquipInArtifactSlot(play, "古刹魔瓶")
            and not Player.hasEquipInArtifactSlot(play, "古魔轩辕剑") then
	...
	["564"] = function(play,mob)      --轩辕剑：30 元档位激活后，每杀 1 只怪累计 +1 切割，上限 88888
	...
	["625"] = function(play,mob)      --嘲天笑地
		...
		_sg_kill_tip(play, (prep.progress_name or prep.name or "任务"), sg_data[key], (prep.need or 0))
		Player.setJsonVarByTable(play, VarCfg["T_各剧情杀怪"], sg_data)
	end,
	...
	["672"] = function(play,mob)      --轮回之路
		...
		local key = "npc_672"
		local need_a, need_b, need_c = 0, 0, 0
		local add_a, add_b, add_c = false, false, false
		if config.details then
			for _, v in ipairs(config.details) do
				if v.a_num then
					need_a = tonumber(v.a_num) or need_a
				end
				if v.b_num then
					need_b = tonumber(v.b_num) or need_b
				end
				if v.c_num then
					need_c = tonumber(v.c_num) or need_c
				end
		...
		if add_a or add_b or add_c then
			local target_name, target_cur, target_need
			if add_a then
				target_name = "系列BOSS"
				target_cur = tonumber(sg_data[key.."_a"] or 0) or 0
				target_need = need_a
			elseif add_b then
				target_name = "六道轮回"
				target_cur = tonumber(sg_data[key.."_b"] or 0) or 0
				target_need = need_b
			elseif add_c then
				target_name = "弱鸡"
				target_cur = tonumber(sg_data[key.."_c"] or 0) or 0
				target_need = need_c
			end
			_sg_kill_multi_tip(play, (config.name or "任务"), "击杀",
				_sg_color_text(target_name .. " ", "#ffd36b") ..
				_sg_progress_text(math.min(target_cur, target_need), target_need))
		end
		Player.setJsonVarByTable(play, VarCfg["T_各剧情杀怪"], sg_data)
	end,
    ...
    ["700"] = function(play,mob)      --第五章：赤焰试炼（当前地图累计击杀300只怪）
        _story5_kill_progress(play, mob, 700)
    end,
	...
	["714"] = function(play,mob)      --第五章：屠龙宝刀（击杀进度+地图掉落冰火龙鳞）
        ...
        if type(need_boss) == "string" and need_boss ~= "" then
            if need_boss == "BOSS" then
                local mob_type = tonumber((guaiwutype and guaiwutype[mob_name]) or 0) or 0
                if mob_type ~= 2 then
                    return
                end
...
-- 9.8-1 end,
	["34"] = function(play,mob)      --筑基丹碎片：二、三大陆普通怪每累计 100 只保底掉落，总上限 30 个碎片
		...
		local dan_count = tonumber(getbagitemcount(play, "筑基丹") or 0) or 0
