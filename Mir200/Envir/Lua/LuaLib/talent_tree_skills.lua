...
local function relation_matches(relation, source, target)
    ...
    local expected = tostring(ELEMENT_NAME[source] or "") .. "克" .. tostring(ELEMENT_NAME[target] or "")
    return expected ~= "克" and tostring(relation.name or "") == expected
end
...
function TalentTreeSkills.sync(play, state)
    ...
    for key, skill in pairs(SKILLS) do
        ...
        if key_active(state, key) then
            ...
            setmagicskillefft(play, getskillname(skill.idx), skill.idx)
