...
local function _xyl_has_lingshou_star(play, star)
    ...
    for i = 1, count do
        local curStar = tonumber(ls_sp[tostring(i)] or 0) or 0
...
local function _xyl_has_zhuri_bow_full(play)
    local where = Player.hasEquipInArtifactSlot(play, "逐日弓")
    if not where then
        return false
    end
    ...
    if type(item_json.abil) == "table" then
        for _, abil in ipairs(item_json.abil) do
            if type(abil) == "table" and tostring(abil.t or "") == "[逐日神秘属性]" and type(abil.v) == "table" then
                return #abil.v >= 9
            end
        end
    end
...
local function _xyl_check_task(play, name)
    ...
    local special = {
        ...
        ["灵兽全一星"] = function(play)
            return _xyl_has_lingshou_star(play, 1)
        end,
        ["灵兽全二星"] = function(play)
