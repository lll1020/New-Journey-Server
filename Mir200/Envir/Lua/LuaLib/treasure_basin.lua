...
local _huangquan_state_var = (VarCfg and VarCfg["A_黄泉降世json"]) or "A17"
local _huangquan_attr_list = "黄泉降世全服祝福"
local _huangquan_player_flag = "N$聚宝盆黄泉降世生效"
...
local function _play_for_online_players(play, showId)
    for _, target in ipairs(getplayerlst() or {}) do
        if target ~= play then
            sendluamsg(target, 107, showId, 0, 0, "")
        end
    end
end
local function _start_huangquan_blessing(ownerName)
    ...
    state.owner = tostring(ownerName or "神秘玩家")
...
local function _broadcast_huangquan(ownerName)
    ownerName = tostring(ownerName or "神秘玩家")
    sendmovemsg("0", 1, 249, 0, 300, 3, "【黄泉降世】玩家《" .. ownerName .. "》唤醒冥河鬼主，全服坠入黑暗！")
    sendmovemsg("0", 1, 249, 0, 270, 3, "【黄泉赐福】持续5分钟：聚宝盆炼化速度+10%，打怪爆率+10%！")
    sendmovemsg("0", 1, 249, 0, 240, 3, "愿黄泉幽光护佑诸位，秘宝临身，诸邪退避！")
end
...
local function _recharge_total(play)
    local totalCharge = _toint(getplaydef(play, VarCfg["U_真实充值"]))
    return totalCharge
end
...
function TreasureBasin.activate(play, reason)
    ...
    if not _has_artifact(play) then
        Player.rwjl(play, {{_artifact_name, 1}}, reason or "聚宝盆", 1, 0)
        changed = true
    end
    if changed then
        _save_state(play, data)
...
function TreasureBasin.getRedState(play, data)
    ...
    if active then
        ...
        local crystal = getbagitemcount(play, "禁元神晶")
        for id, _ in ipairs(_forbidden) do
...
local function _build_payload(play)
    ...
    return {
        ...
        real_charge = _toint(getplaydef(play, VarCfg["U_真实充值"])),
...
local function _claim_forbidden_reward(play, npcid, id)
    ...
    sendluamsg(play, 107, showId, 0, 0, "")
    _play_for_online_players(play, showId)
    local cfg = _forbidden[showId]
    local ownerName = tostring(getbaseinfo(play, 1) or "神秘玩家")
    local onlineCount = 0
    if showId == 1 then
        onlineCount = _mail_online_players(
            "天地异象·全服祝福",
            "玩家【" .. ownerName .. "】释放禁器技能【天地异象】，为全服在线勇士送来财运祝福。愿诸位财源广进，征途满载！",
            function()
                return {{"金币", math.random(100000, 300000)}, {"元宝", math.random(1000, 3000)}}
            end
        )
        sendmovemsg("0", 1, 253, 0, 280, 2, "【天地异象】玩家《" .. ownerName .. "》赐福全服在线勇士，金币元宝奖励已发送至邮件！")
    elseif showId == 2 then
        _start_huangquan_blessing(ownerName)
        onlineCount = _mail_online_players(
            "黄泉降世·全服祝福",
            "玩家【" .. ownerName .. "】释放禁器技能【黄泉降世】。全服进入5分钟黑暗状态，期间聚宝盆炼化速度提高10%、打怪爆率提高10%。愿黄泉幽光护佑诸位，秘宝临身，诸邪退避！",
            nil
        )
    ...
    elseif showId == 3 then
        onlineCount = _mail_online_players(
            "万灵朝凤·全服祝福",
            "玩家【" .. ownerName .. "】释放禁器技能【万灵朝凤】，为全服在线勇士送来神凰祝福。愿凤鸣九天，鸿运长伴！",
            {{"凤凰之瞳", 5}}
        )
        sendmovemsg("0", 1, 253, 0, 280, 2, "【万灵朝凤】玩家《" .. ownerName .. "》赐福全服在线勇士，凤凰之瞳已发送至邮件！")
    end
    _save_state(play, data)
    Player.sendmsgEx(play, "成功释放外显禁器技能：#57|【" .. tostring(cfg.name or "禁器") .. "】#218|，已祝福在线玩家#57|【" .. tostring(onlineCount) .. "】#218|名")
    _refresh_item_bar(play)
