...
function npc.link(play,npcid,ew,aid,data)
    ...
    if ew == 1 then -- 做酒水
        ...
        if curzuiyi >= _config.max_zuiyi then
            Player.sendmsgEx(play, string.format("你的醉意值已达上限#57|【%d】#218|，无法继续饮用酒水#57", _config.max_zuiyi))
            return
        end
        ...
        local reward = _config.details[randomNum] and _config.details[randomNum].cost and _config.details[randomNum].cost[1]
        if not reward or not reward[1] then
            Player.sendmsgEx(play, "酒水配置错误#57")
            return
        end
        giveitem(play, reward[1], tonumber(reward[2] or 1) or 1)
        Player.sendmsgEx(play, string.format("酿造成功，获得|【%s】#218|*%d#57，使用后可增加醉意值#57", tostring(reward[1]), tonumber(reward[2] or 1) or 1))
        sendluamsg(play,100,npcid,1,0,tbl2json({num = getplaydef(play, VarCfg["J_醉意值"])}))
        
    elseif ew == 2 then -- 开启吃货币
        if hasbuff(play,20103) then
            delbuff(play,20103)
            Player.sendmsgEx(play, "你关闭了|【醉酒狂魔舞】#218")
        else
            local curzuiyi = getplaydef(play, VarCfg["J_醉意值"])
            if curzuiyi < _config.max_zuiyi then
                Player.sendmsgEx(play, string.format("你的醉意值未达上限#57|【%d】#218|，无法开启醉酒狂魔舞#57", _config.max_zuiyi))
                return
            end
            ...
            addbuff(play, 20103)
            Player.sendmsgEx(play, "你开启了|【醉酒狂魔舞】#218|，接下来每秒钟将自动消耗|【200元宝】#218")
        end
        
    
    --     if curzuiyi >= _config.max_zuiyi then
    --         Player.sendmsgEx(play, string.format("你的醉意值已达上限|%d#218|，无法继续饮用酒水#57", _config.max_zuiyi))
    --         return
    --     end
    ...
    --     Player.sendmsgEx(play, string.format("你饮用了|%s#218|，醉意值增加了|%d#218|，当前醉意值为|%d#218", _config.details[json_data.idx].name, addzuiyi, curzuiyi + addzuiyi))
    --     sendluamsg(play,100,npcid,1,0,tbl2json({num = getplaydef(play, VarCfg["J_醉意值"])}))
