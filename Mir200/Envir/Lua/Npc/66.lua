...
function npc.link(play,npcid,ew,aid,data)
    ...
    if ew == 1 then -- 福娃猜拳
        ...
        if result == 1 then
            ...
            Player.sendmsgEx(play, string.format("恭喜你在本轮福娃猜拳中获胜，你出的是【%s】，福娃出的是【%s】，额外获得【%d】切割", player_choice_name, npc_choice_name, cut_gain))
        elseif result == 2 then
            T_data.losses = T_data.losses + 1
            Player.sendmsgEx(play, string.format("很遗憾你在本轮福娃猜拳中失败了#57，你出的是【%s】，福娃出的是【%s】，额外获得【%d】切割", player_choice_name, npc_choice_name, cut_gain))
        else
            Player.sendmsgEx(play, string.format("本轮福娃猜拳平局，你出的是【%s】，福娃出的是【%s】，额外获得【%d】切割", player_choice_name, npc_choice_name, cut_gain))
        end
