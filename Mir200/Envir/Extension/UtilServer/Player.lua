...
function Player.title_give(actor, title_name) --给称号
    if not checktitle(actor, title_name) then
        ...
        if not buff_applied and title_name == "诸邪退散" and Buff[563] then
            Buff[563](actor,1)
        end
        Player.trySyncSecondContinentXyl(actor)
...
function Player.title_del(actor, title_name) --删称号
    if checktitle(actor, title_name) then
        ...
        if had_huijie_title and title_name == "诸邪退散"
...
local function _dl_check(actor, dl)
    ...
    elseif dl == 6 then
        ...
        if story_ok and zslv >= 50 and _dl_has_all_destiny(actor) then
            _dl_mark_unlocked(actor, 6)
            return true
        end
        return false, "需五大陆剧情点达到50点、完成五大陆转生并完成天道命盘后才可进入六大陆"
    elseif dl == 7 then
