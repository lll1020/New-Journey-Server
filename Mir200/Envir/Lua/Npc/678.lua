...
local _attr_list_name = "后土娘娘"

local function _build_detail_attr_map(jq_data)
    local attrs = {}
    local details = (_config and _config.details) or {}
    for i = 1, #details do
        local cfg = details[i]
        if cfg and cfg.attr and jq_data["npc_678_"..i] and jq_data["npc_678_"..i] >= 2 then
            for _, it in ipairs(cfg.attr) do
...
local function _apply_detail_attrs(play)
    local jq_data = Player.getJsonTableByVar(play, VarCfg.T_dljq)
    local attrs = _build_detail_attr_map(jq_data)
...
local function _ensure_kill_listener(play)
    local jq_data = Player.getJsonTableByVar(play, VarCfg.T_dljq)
    local key = "npc_678"
    if tonumber(jq_data[key] or 0) ~= 1 then
        return
    end
    if tonumber(jq_data[key.."_2"] or 0) >= 2 then
        return
    end
...
function Login_jq_678(play)
    _apply_detail_attrs(play)
    _ensure_kill_listener(play)
end
GameEvent.add(EventCfg.onLogin, Login_jq_678, "Login_后土娘娘")
