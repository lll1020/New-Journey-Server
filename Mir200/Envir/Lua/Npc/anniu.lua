...
local function _ywl_try_open_submit_npc_if_ready(play, shuju)
    -- 热血的友情在异闻录中保持正常传送，不再提前拦截为直接打开提交界面。
    return false
end
...
npc[504] = function(play, p2, p3, data) --快人一步
    ...
    elseif p2 == 1 then
        if getflagstatus(play, VarCfg.BS_mztq) == 0 then
            sendluamsg(play, 101, 999, tonumber((teshudata["anniu_504"] or {}).price or 58) or 58, 21, "")
        else
...
local function _activity507_is_open(p3)
    ...
    elseif p3 == 7 then
        local txzrMinute = tonumber(getsysvar(VarCfg["G_天选之人"][1]) or 0) or 0
        local txzrRound = tonumber(getsysvar(VarCfg["G_天选之人"][2]) or 0) or 0
        return txzrRound < 4 and txzrMinute > 0 and txzrMinute < 30
    elseif p3 == 8 then
...
local function _activity507_open_state_payload()
    return tbl2json({
        ...
        [7] = _activity507_is_open(7) and 1 or 0,
...
npc[507] = function(play, p2, p3, msgData) --活动面板
    if p2 == 0 then
        ...
        local payload = {
            ...
            now_minute = (tonumber(os.date("%H")) or 0) * 60 + (tonumber(os.date("%M")) or 0),
            now_weekday = tonumber(os.date("%w")) or 0,
            ...
            mskh = mskh_tb,
            txzr = {
                minute = tonumber(getsysvar(VarCfg["G_天选之人"][1]) or 0) or 0,
                round = tonumber(getsysvar(VarCfg["G_天选之人"][2]) or 0) or 0,
                open = _activity507_is_open(7) and 1 or 0,
            },
