...
local _config = Guard.getConfig("npc_624")

local _target_npc = {map = "禁忌之海", id = 626, x = 74, y = 67}
...
function npc.link(play,npcid,ew,aid)
    ...
    if ew == 1 then
        ...
        if not jq_data[key] or jq_data[key] == 0 then
            ...
            shaguai.jia(play, _config.shaguai_id or 624)
        ...
        if jq_data[key] == 1 then
            if sg_data[key] and sg_data[key] >= (_config.num or 0) then
                ...
                if rewards then
                    Player.rwjl(play, rewards, tostring(_config.name or "npc_624") .. "_reward", 1)
                end
